...
export const PatientDashboard: React.FC = () => {
   ...
   // Filter data for current patient - records already filtered by backend, use directly
   const myRecords = records; // Backend already returns only current user's records
   ...
   const handleUploadSubmit = async (e: React.FormEvent) => {
      ...
      if (selectedFile) {
         try {
            await uploadRecord(selectedFile, uploadForm.fileType);
            setShowUploadModal(false);
            setUploadForm({ title: '', description: '', fileType: 'PDF' });
                  ...
                  <form onSubmit={handleUploadSubmit} className="p-6 space-y-4">
                     <div>
                        ...
                        <div className="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center hover:bg-slate-50 transition-colors">
                           ...
                           />
                           {selectedFile && <p className="text-xs text-green-600 mt-2 font-medium">Selected: {selectedFile.name}</p>}
                        </div>
                     </div>
                     ...
                     <button type="submit" disabled={!selectedFile} className="w-full bg-primary disabled:bg-slate-300 text-white py-2 rounded-lg font-medium hover:bg-sky-600">
                        Encrypt & Upload
                     </button>
                  </form>
               </div>
